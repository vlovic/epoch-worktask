...
PARAMS = {"search_query": "cat:cs.AI", "max_results": str(int(RESULTS_PER_QUERY)), "sortBy": "submittedDate"}
for i in range(int(round(TOTAL_RESULTS / RESULTS_PER_QUERY))): 
    print(i)
    PARAMS["start"] = int(round(RESULTS_PER_QUERY * i))
    num_entries = 0
    counter = 0
    while num_entries != RESULTS_PER_QUERY: # Sometimes the API request returns 0 entries, in which case we try again.
